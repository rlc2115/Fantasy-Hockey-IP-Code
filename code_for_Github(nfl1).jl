...
function one_lineup_Type_1(players, qbs, dst, lineups, num_overlap, num_players, num_qbs, num_dst, rbs, wrs, tes, num_teams, players_teams, quarterback_opponents, team_lines, num_lines, P1_info)
    m = Model(solver=GLPKSolverMIP())
    ...
    # At least 3 different teams for the 7 players constraint
    @defVar(m, used_team[i=1:num_teams], Bin)
    @addConstraint(m, constr[i=1:num_teams], used_team[i] <= sum{players_teams[t, i]*players_lineup[t], t=1:num_players})
    @addConstraint(m, sum{used_team[i], i=1:num_teams} >= 3)


    # No qbs going against players constraint
    @addConstraint(m, constr[i=1:num_qbs], 6*qbs_lineup[i] + sum{quarterback_opponents[k, i]*players_lineup[k], k=1:num_players}<=6)


    # Must have at least one complete line in each lineup
    @defVar(m, line_stack[i=1:num_lines], Bin)
    @addConstraint(m, constr[i=1:num_lines], 3*line_stack[i] <= sum{team_lines[k,i]*players_lineup[k], k=1:num_players})
    ...
    @addConstraint(m, constr[i=1:num_lines], 2*line_stack2[i] <= sum{team_lines[k,i]*players_lineup[k], k=1:num_players})
    @addConstraint(m, sum{line_stack2[i], i=1:num_lines} >= 2)


    # Overlap Constraint
    @addConstraint(m, constr[i=1:size(lineups)[2]], sum{lineups[j,i]*players_lineup[j], j=1:num_players} + sum{lineups[num_players+j,i]*qbs_lineup[j], j=1:num_qbs} <= num_overlap)


    # Objective
    @setObjective(m, Max, sum{players[i,:Projection]*players_lineup[i], i=1:num_players} + sum{qbs[i,:Projection]*qbs_lineup[i], i=1:num_qbs} )
...
function one_lineup_Type_2(players, qbs, dst, lineups, num_overlap, num_players, num_qbs, num_dst, rbs, wrs, tes, num_teams, players_teams, quarterback_opponents, team_lines, num_lines, P1_info)
    m = Model(solver=GLPKSolverMIP())

    # Variable for players in lineup
    @defVar(m, players_lineup[i=1:num_players], Bin)

    # Variable for quarterback in lineup
    @defVar(m, qbs_lineup[i=1:num_qbs], Bin)
...
function one_lineup_Type_3(players, qbs, dst, lineups, num_overlap, num_players, num_qbs, num_dst, rbs, wrs, tes, num_teams, players_teams, quarterback_opponents, team_lines, num_lines, P1_info)
    m = Model(solver=GLPKSolverMIP())
    ...
    @addConstraint(m, sum{sum{tes[i]*P1_info[i,j]*players_lineup[i], i=1:num_players}, j=1:num_teams} ==  sum{tes[i]*players_lineup[i], i=1:num_players})
...
function one_lineup_Type_4(players, qbs, dst, lineups, num_overlap, num_players, num_qbs, num_dst, rbs, wrs, tes, num_teams, players_teams, quarterback_opponents, team_lines, num_lines, P1_info)
    m = Model(solver=GLPKSolverMIP())
    ...
    # exactly 3 different teams for the 7 players constraint
    @defVar(m, used_team[i=1:num_teams], Bin)
    @addConstraint(m, constr[i=1:num_teams], used_team[i] <= sum{players_teams[t, i]*players_lineup[t], t=1:num_players})
    @addConstraint(m, constr[i=1:num_teams], sum{players_teams[t, i]*players_lineup[t], t=1:num_players} <= 6*used_team[i])
    @addConstraint(m, sum{used_team[i], i=1:num_teams} == 3)
...
function one_lineup_Type_5(players, qbs, dst, lineups, num_overlap, num_players, num_qbs, num_dst, rbs, wrs, tes, num_teams, players_teams, quarterback_opponents, team_lines, num_lines, P1_info)
    m = Model(solver=GLPKSolverMIP())
